...
import { ArrowUpDown, Search, X, Zap, Lightbulb } from "lucide-react";
import { DataTablePagination } from "@/components/dashboard/TablePagination";
import Loader from "@/components/ui/loading-spiner";
import { FastMovingGoodItem, FastMovingFilters } from "@/types/analysis";
import { useFastMovingGoods } from "@/server-queries/analysisQueries";
import { useAuthStore, useAuthStoreAccess } from "@/stores/authStore";

const columnHelper = createColumnHelper<FastMovingGoodItem>();
...
export default function FastMovingGoodsPage() {
  ...
  const MOCK_FILTERS: FastMovingFilters = {
    store_id: storeIds[0],
    days: 3650,
    limit: 50,
  };
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex justify-center">
              <Badge className={`font-bold ${medalClass}`}>#{rank}</Badge>
            </div>
          );
        },
      }),
      ...
      <div className="space-y-6 p-2">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Fetching fast moving goods&hellip;
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input
              placeholder="Search fast moving goods..."
              disabled
                  ...
                  >
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Preparing fast moving goods&hellip;
                      </p>
                    </div>
                  </TableCell>
                </TableRow>
  ...
  if (isError || !data.length) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <p className="text-muted-foreground">No fast moving goods found</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 p-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Fast Moving Goods</h1>
        <p className="text-muted-foreground text-sm mt-1">
          {response?.total_items ?? 0} top-moving items &mdash; last{" "}
